...
export function OfficialDmcCard({ dmc, series = [], diagnostic }: { dmc: DmcRow; series?: DmcSeriesSummary[]; diagnostic?: DmcHydraulicMonthlyRow }) {
  const available = series.length > 0 || Boolean(diagnostic)
  return (
    <article className="dmc-card">
      ...
      {series.length > 0 ? <div className="compact-list dmc-series-list">{series.map((item) => <div key={`${item.importId}-${item.channelType}`}><span><strong>{channelLabels[item.channelType] ?? item.channelType}</strong><small className="cell-note">{new Date(item.firstReading).toLocaleDateString('pt-BR', { timeZone: 'America/Sao_Paulo' })} → {new Date(item.lastReading).toLocaleDateString('pt-BR', { timeZone: 'America/Sao_Paulo' })}</small></span><strong>{item.rawCount.toLocaleString('pt-BR')} RAW · {item.unit}{item.normalizedUnit && item.normalizedUnit !== item.unit ? ` → ${item.normalizedUnit === 'l_s' ? 'L/s' : item.normalizedUnit}` : ''} · {item.gapCount} gaps · {item.coveragePercent?.toLocaleString('pt-BR', { maximumFractionDigits: 2 }) ?? '—'}%</strong></div>)}</div> : !diagnostic && <p>{dmc.notes ?? 'Nenhuma série hidráulica oficial importada.'}</p>}

...
import { decodeBytes, detectDelimiter, detectEncoding, importContextKey, normalizeHeader, parseDelimitedText, parseLocaleNumber, parseLocalTimestamp, prevalidate, sha256Hex, suggestMapping } from './parser'
...
const mappings = (channel: ColumnMapping['channelType'] = 'PRESSURE_PC', unit = 'mca'): ColumnMapping[] => [
  { index: 0, headerOriginal: 'Data e hora', headerNormalized: 'data e hora', channelType: 'TIMESTAMP', unit: null, confidence: 'HIGH' },
  { index: 1, headerOriginal: 'Pressão 1 (mca)', headerNormalized: 'pressao 1 mca', channelType: channel, unit, confidence: 'HIGH' },
]
const table = (rows: unknown[][]): ParsedTable => ({ headers: ['Data e hora', 'Pressão 1 (mca)'], rows, encoding: 'UTF-8', delimiter: ';' })

...
const DMC_CHANNELS: ImportChannel[] = ['PRESSURE_PC', 'PRESSURE_UPSTREAM', 'PRESSURE_DOWNSTREAM', 'FLOW']
...
const formatNumber = (value: number | null) => value === null ? '—' : value.toLocaleString('pt-BR', { maximumFractionDigits: 2 })
...
export function ImportacoesPage() {
  ...
  const mappingsFor = useCallback((parsed: ParsedTable, selectedChannel = channel, selectedUnit = unit) => {
    const suggested = suggestMapping(parsed.headers, sourceMode === 'DMC' ? selectedDmc?.pc_channel : null, sourceMode === 'DMC' ? 'PRESSURE_PC' : 'PRESSURE_SUPPLY', parsed.hasHeader === true)
    if (sourceMode === 'DMC') return suggested.map((item) => DMC_CHANNELS.includes(item.channelType) ? { ...item, unit: defaultUnit(item.channelType) } : item)
  ...
  }, [channel, selectedDmc?.pc_channel, sourceMode, unit])
  ...
  function chooseSource(next: SourceMode) { setSourceMode(next); setStep(0); clearFile() }
  ...
  function changeMapping(index: number, next: ImportChannel) {
    setValidation(null)
    setMappings((current) => current.map((item) => item.index === index ? { ...item, channelType: next, unit: sourceMode === 'DMC' ? defaultUnit(next) : next === channel ? unit : null } : item))
  }
  ...
  }, [bytes, channel, file, hash, mappings, source, sourceMode, table, unit])
  ...
  return <>
    ...
    <History history={history} loading={historyLoading} dmcNames={dmcNames} select={setSelectedHistory} />
...
function PrevalidationMetrics({ value, mappings, table }: { value: PrevalidationResult; mappings: ColumnMapping[]; table: ParsedTable }) {
  ...
  const channels = mappings.filter((item) => isDataChannel(item.channelType)).map((mapping) => { const measurements = value.measurements.filter((item) => item.columnIndex === mapping.index); const numeric = measurements.map((item) => item.rawValue).filter((item): item is number => item !== null); const flags = value.flags.filter((flag) => flag.columnIndex === mapping.index); const channelQuality = summarizeFlags(flags.map((flag) => ({ flag_type: flag.flagType, severity: flag.severity, details: flag.details }))); return { mapping, rawCount: measurements.length, minimum: numeric.length ? Math.min(...numeric) : null, maximum: numeric.length ? Math.max(...numeric) : null, quality: channelQuality, coverage: calculateCoverage(value.firstReading, value.lastReading, value.predominantCadenceMinutes, measurements.length) } })

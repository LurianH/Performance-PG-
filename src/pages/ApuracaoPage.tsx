...
export function ApuracaoPage() {
  ...
  return (
    <>
      ...
      {isMockMode && <MockNotice>valores transcritos do protótipo; a aplicação não executa VP, redução ou atingimento.</MockNotice>}
      <div className="grid-main">
        <Card className="wide-card">{isMockMode ? <div className="table-wrap"><table><thead><tr><th>Competência</th><th>VP</th><th>Redução</th><th>% meta</th><th>Origem/status</th></tr></thead><tbody>{performanceMonthsMock.map((month) => <tr key={month.competence}><td><strong>{month.competence}</strong></td><td>{number.format(month.vp)}</td><td>{number.format(month.reduction)}</td><td>{month.attainment.toLocaleString('pt-BR')}%</td><td><StatusBadge status={month.status} /></td></tr>)}</tbody></table></div> : <DataState loading={months.loading} error={months.error} empty="Não há competências oficiais cadastradas. VD e VCM serão carregados a partir da fonte oficial antes do cálculo de VP." />}</Card>

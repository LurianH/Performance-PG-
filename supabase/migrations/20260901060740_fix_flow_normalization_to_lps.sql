-- Correção aditiva: a unidade analítica canônica de FLOW é L/s.
-- RAW permanece imutável; somente a projeção validada/normalizada é ajustada.
...
select
  ...
  rm.raw_value,
  case
    when rm.channel_type = 'FLOW' and lower(rm.unit) in ('m3/h', 'm3_h', 'm3h')
      then rm.raw_value / 3.6
    when rm.channel_type = 'FLOW' and lower(rm.unit) in ('l/s', 'l_s', 'ls')
      then rm.raw_value
    when rm.channel_type in (
      'PRESSURE_PC', 'PRESSURE_UPSTREAM', 'PRESSURE_DOWNSTREAM', 'PRESSURE_SUPPLY'
    ) and lower(rm.unit) = 'mca'
      then rm.raw_value
    else null
  end as normalized_value,
  rm.unit as raw_unit,
  case
    when rm.channel_type = 'FLOW' and lower(rm.unit) in (
      'm3/h', 'm3_h', 'm3h', 'l/s', 'l_s', 'ls'
    ) then 'l_s'
    when rm.channel_type in (
      'PRESSURE_PC', 'PRESSURE_UPSTREAM', 'PRESSURE_DOWNSTREAM', 'PRESSURE_SUPPLY'
    ) and lower(rm.unit) = 'mca' then 'mca'
    else null
...
comment on view public.validated_measurements is
  'Explica validade sem alterar RAW. FLOW é normalizado canonicamente para L/s; pressões mca permanecem mca; unidades não suportadas não são convertidas.';
